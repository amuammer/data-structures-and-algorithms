...
const mapCurrentEvents = () => {
  return currentEvents.news.map((a) => new Event(a));
}
...
const returnNames = (arr) => arr.reduce((acc, el) => {
  acc.push(el.name);
  return acc;
}, []);
...
const reversedString = (str) => str.split("").reduce((acc, cv) => cv+acc, []);
