...
const createServer = () => {
  ...
  app.get("/aboutme", (req, res) => {
    res.status(200).send({ data: "adnan-NodeJS" });
  });
...
const count = (value, input) => {
  return input.reduce((count, currentNumber) => {
    count += currentNumber.filter((e) => e === value).length;
    return count;
  }, 0);
};
...
const totalSum = (input) => {
  return input.reduce((totalSum, array) => {
    const arraySum = array.reduce((innerSum, currentNumber) => {
      return innerSum + currentNumber;
    }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.reduce((numbersCount, currentNumber) => {
    currentNumber = currentNumber.filter((number)=> {
      return typeof number === 'number'
    })
      .filter((number)=>{return number % 5 === 0;}
      ).map(e => Math.pow(2, e));
